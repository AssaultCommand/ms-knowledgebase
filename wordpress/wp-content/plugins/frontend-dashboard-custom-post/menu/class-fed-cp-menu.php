...
if ( ! class_exists( 'Fed_Cp_Menu' ) ) {
	...
	class Fed_Cp_Menu {
		...
		public function __construct() {
			...
			add_action( 'fed_enqueue_script_style_frontend', array( $this, 'fed_cp_enqueue_script_style_frontend' ) );
		...
		public function fed_cp_enqueue_script_style_admin() {
			wp_enqueue_script( 'fed_cp_script', plugins_url( '/assets/fed_cp_script.js', FED_CP_PLUGIN ), array() );
		}

		/**
		 * Enqueue Script and style in Frontend
		 */
		public function fed_cp_enqueue_script_style_frontend() {
			wp_enqueue_style( 'fed_cp_style', plugins_url( '/assets/fed_cp_style.css', FED_CP_PLUGIN ), array() );
		}
		...
		public function fed_cp_frontend_dashboard_menu_container( $request, $menu_items ) {
			if ( $menu_items['menu_request']['menu_type'] === 'post' ) {
				...
				$menu_name  = $this->getMenuNameByPostType( $post_menus[ $menu_items['menu_request']['menu_slug'] ],
					$post_type );
				$menu_icon  = $this->getMenuIconByPostType( $post_menus[ $menu_items['menu_request']['menu_slug'] ],
					$post_type );
					...
					<div class="panel panel-primary fed_dashboard_item active">
						...
						<div class="panel-body fed_dashboard_panel_body">
							...
							if ( ! isset( $request['post_status'] ) && ! isset( $request['post_id'] ) ) {
								$this->fed_display_dashboard_view_post_list( $menu_items['menu_request']['menu_slug'],
									$menu );
							}
							?>
						</div>
...
//					'input' => fed_cp_customize_post_for_user_role( $request, $index, $user_roles )
		...
		public function fed_dashboard_delete_post_by_id_fn() {
			...
			if ( ! wp_verify_nonce( $post['fed_dashboard_delete_post_by_id'], 'fed_dashboard_delete_post_by_id' ) ) {
				wp_send_json_error( array(
					'message' => array(
						__( 'Invalid Request, Please reload the page and try again', 'frontend-dashboard-custom-post' )
					)
				) );
								...
								<div class="row pull-right">
									<?php if ( fed_cp_is_user_can_view_post( $post_type ) ) { ?>
										<div class="col-xs-4 col-sm-4">
											<a class="btn btn-warning" target="_blank" href="<?php echo get_permalink( (int) $single_post->ID ) ?>">
												<i class="fa fa-eye"></i>
											</a>
										</div>
									<?php } ?>
		...
		private function fed_cp_frontend_dashboard_edit_post_by_id( $post_id, $menu ) {
			...
			if ( fed_cp_is_user_can_edit_post( $post->post_type ) && $post !== null && $post->post_author == $user->ID ) {
				...
				<div class="row">
					<div class="col-md-6">
						<a class="btn btn-primary" href="<?php echo esc_url( site_url() . remove_query_arg( 'post_id' ) ); ?>">
							<i class="fa fa-mail-reply"></i>
							Back
							to <?php echo $menu['query']['menu_items'][ $menu['query']['menu_request']['menu_slug'] ]['menu']; ?>
						</a>
